...
const DEFAULT_INPUT_PATH = path.resolve(process.cwd(), "data/news.sample.json");
const DEFAULT_OUTPUT_PATH = path.resolve(process.cwd(), "data/news.json");
const DEFAULT_SOURCE = "Security News Adapter";
...
function normalizeRow(row, index) {
  ...
  const normalized = {
    id: String(row?.id || `news-${String(index + 1).padStart(3, "0")}`),
    title: String(row?.title || "Untitled news item"),
