...
const DEFAULT_INPUT_PATH = path.resolve(process.cwd(), "data/kev.sample.json");
const DEFAULT_OUTPUT_PATH = path.resolve(process.cwd(), "data/kev.json");
const DEFAULT_SOURCE = "CISA KEV Adapter";
...
function normalizeRow(row, index) {
  ...
  const normalized = {
    id: String(row?.id || `kev-${String(index + 1).padStart(3, "0")}`),
    title: String(row?.title || "Untitled KEV item"),
